...
| Monitoring Service metrics | Minimal | `mlids_*` counters + histograms + gauges |
...
| Model fallback | ML Tracking only | ML Tracking → local joblib cache |
...
| Monitoring | Monitoring Service (custom metrics) |
...
├── main.py                  FastAPI app, /predict, ModelManager (ML Tracking + cache fallback)
...
- **ML Tracking has a local cache fallback** — if ML Tracking is unreachable on startup, the model is loaded from `MODEL_CACHE_DIR/model_cache.joblib`.
